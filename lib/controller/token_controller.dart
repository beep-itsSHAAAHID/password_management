...
class TokenController {
  ...
  Future<int> generateTokenForBooking(
      String doctorId, String session, DateTime selectedDate) async {
    String dateString =
    ...
    return firestore.runTransaction<int>((transaction) async {
      ...
      DocumentReference doctorSessionRef =
          firestore.collection('doctor_sessions').doc('$doctorId-$session');

      // Try to read the current state of the doctor's session
      DocumentSnapshot doctorSessionSnapshot =
          await transaction.get(doctorSessionRef);
